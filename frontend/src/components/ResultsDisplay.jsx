...
const ResultsDisplay = ({ predictionResult }) => {
  ...
  useEffect(() => {
    ...
    // Check if there was a save error in the prediction result
    if (predictionResult.save_error) {
      console.error(
        "Save error detected in prediction result:",
        predictionResult.error_message
      );
      setPatientError(
        predictionResult.error_message ||
          "Failed to save patient data. Results are temporary."
      );
      return;
    }
    ...
    const verifyPatient = async () => {
      try {
        ...
        try {
          ...
          console.log(`Verifying patient with timestamp ${timestamp}`);

          const response = await axios.get(
            `http://localhost:8083/api/patients/${predictionResult.patient_id}`,
            {
              params: {
        ...
        const timestamp = new Date().getTime();
        console.log(`Checking patient with timestamp ${timestamp}`);

        const checkResponse = await axios.get(
          `http://localhost:8083/api/patients/check/${predictionResult.patient_id}`,
          {
            params: {
              t: timestamp, // Add cache-busting parameter
            },
  ...
  useEffect(() => {
    if (
      ...
      console.log("SHAP values:", {
        ...
        feature_names: shapData.feature_names.slice(0, 5) + "...", // Show first 5 names
        values_length: shapValues.length,
        values_sample: shapValues.slice(0, 5), // Show first 5 values
      });
        ...
        data: {
          labels: topFeatures.map((f) => {
            ...
            const nameMap = {
              ...
              prior_event_severity: "Prior Cardiac Event",
              time_since_event: "Time Since Event",
              nt_probnp: "NT-proBNP Biomarker",
              chest_pain_type: "Chest Pain Type",
              pvc_count: "PVC Count",
              qt_prolongation: "QT Prolongation",
              af_detected: "Atrial Fibrillation",
              tachycardia_detected: "Tachycardia",
  ...
  const riskLevel =
    ...
    (predictionResult.prediction >= 0.28
      ? "High"
      : predictionResult.prediction >= 0.12
      ? "Medium"
      : "Low");
