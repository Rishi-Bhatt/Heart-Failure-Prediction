...
This script tests the fixed model to ensure it works correctly with
...
def test_fixed_model():
    """
    Test the fixed model with different risk profiles
    """
    print("\nTesting Fixed Model with Different Risk Profiles")
    print("=" * 60)
        ...
        try:
            ...
            try:
                if shap_values and 'values' in shap_values and 'feature_names' in shap_values:
                    # Make sure values and feature_names have the same length
                    values = shap_values['values']
                    feature_names = shap_values['feature_names']

                    # Trim to the shorter length
                    min_length = min(len(values), len(feature_names))
